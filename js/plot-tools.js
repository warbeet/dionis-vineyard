...
function showRobotRoute(plotId) {
  ...
  if (!route || !route.points.length) {
    toast('Нет кустов с GPS. Сначала рассчитайте GPS автоматически.', 'error');
    return;
  }

  // Рендерим линию маршрута на Яндекс-карте
  const map = yandexMaps[plotId];
  if (!map || typeof ymaps === 'undefined') {
    toast('Сначала откройте карту участка', 'error');
    return;
  }

  // Удаляем предыдущий маршрут
  if (window._robotRouteLayer) {
    try { map.geoObjects.remove(window._robotRouteLayer); } catch(e) {}
  }
  ...
  window._robotRouteLayer = polyline;
...
// =========== ВИЗУАЛЬНЫЙ РЕДАКТОР РЯДОВ НА КАРТЕ ===========
//
// Позволяет рисовать ряды линиями прямо на Яндекс-карте.
// При двойном клике в карту — начать новый ряд (точка А). Ещё клик — точка Б.
// Из этих 2-х точек создаётся ряд с автоматическим расчётом позиций.

let drawingMode = false;
let currentDrawRowStart = null;
let drawnRowsLayer = null;

function toggleRowDrawingMode(plotId) {
  drawingMode = !drawingMode;
  const map = yandexMaps[plotId];
  if (!map) { toast('Сначала откройте карту', 'error'); return; }

  if (drawingMode) {
    toast('✏️ Режим рисования: кликните 2 точки — начало и конец ряда', 'success');
    map.events.add('click', onDrawClick);
    if (!drawnRowsLayer) {
      drawnRowsLayer = new ymaps.GeoObjectCollection();
      map.geoObjects.add(drawnRowsLayer);
    }
  } else {
    toast('Режим рисования выключен', 'info');
    map.events.remove('click', onDrawClick);
    currentDrawRowStart = null;
  }
  // Обновим кнопку
  const btn = document.getElementById('draw-rows-btn-' + plotId);
  if (btn) btn.textContent = drawingMode ? '🛑 Стоп рисование' : '✏️ Рисовать ряды';
}

function onDrawClick(e) {
  const coords = e.get('coords');
  const plotId = Object.keys(yandexMaps).find(id => yandexMaps[id] === e.get('target'));
  if (!plotId) return;
  const plot = data.plots.find(p => p.id === plotId);
  if (!plot) return;

  if (!currentDrawRowStart) {
    // Первый клик — начало ряда
    currentDrawRowStart = coords;
    // Маркер
    const placemark = new ymaps.Placemark(coords, { hintContent: 'Начало ряда' }, {
      preset: 'islands#greenDotIconWithCaption', iconCaption: 'A'
    });
    drawnRowsLayer.add(placemark);
    toast('📍 Точка A. Кликните вторую точку (B) — конец ряда', 'info');
  } else {
    // Второй клик — конец ряда. Создаём ряд.
    const a = currentDrawRowStart, b = coords;
    const latDiff = (b[0] - a[0]) * 111320;
    const lngDiff = (b[1] - a[1]) * 111320 * Math.cos(a[0] * Math.PI / 180);
    ...
    const newRow = {
      ...
      number: typeof formatRowNumber === 'function' ? formatRowNumber(rowIdx, plot.row_naming || 'numbers') : rowIdx,
      positions_count: positions,
      start_position: 1,
      start_gps: { lat: a[0], lng: a[1] },
      end_gps: { lat: b[0], lng: b[1] },
    ...
    // Линия ряда
    const line = new ymaps.Polyline([a, b], {
      hintContent: `Ряд ${newRow.number}: ${positions} позиций, ${lengthM.toFixed(1)} м`
    }, {
      strokeColor: '#6b8e5a', strokeWidth: 4, opacity: 0.8
    });
    drawnRowsLayer.add(line);

    const endMark = new ymaps.Placemark(b, { hintContent: `Конец ряда ${newRow.number}` }, {
      preset: 'islands#redDotIconWithCaption', iconCaption: 'B'
    });
    drawnRowsLayer.add(endMark);

    toast(`✅ Ряд ${newRow.number}: ${positions} позиций, ${lengthM.toFixed(1)} м`, 'success');
    currentDrawRowStart = null;
  }
}

function clearDrawnRows(plotId) {
  if (drawnRowsLayer && yandexMaps[plotId]) {
    drawnRowsLayer.removeAll();
  }
  currentDrawRowStart = null;
}
